def parseLineStep1(line)
  game, sets = line.split ':'

  return {
    id: game.split[1].to_i,
    sets: sets.split(';').map { |set| set.split(',').map { |cubes| s = cubes.split; { color: s[1], count: s[0].to_i } } },
  }
end

def parseLineStep2(line)
  game, sets = line.split ':'

  return {
    id: game.split[1].to_i,
    sets: sets.split(';').map { |set| set.split(',').map { |cubes| s = cubes.split; [s[1], s[0].to_i] }.to_h },
  }
end
...
def solveStep1(input)
  ...
  p input
    .split("\n")
    .map { |line| parseLineStep1(line) }
    .select { |game| game[:sets].all? { |set| set.all? { |cubes| cubes[:count] <= MAX[cubes[:color]] } } }
    .map { |game| game[:id] }
    .sum
end
...
def solveStep2(input)
  ...
  p input
    .split("\n")
    .map { |line| parseLineStep2(line) }
    .map { |game| MAX.keys.map { |key| game[:sets].map { |set| set.fetch(key, 0) }.max }.reduce(:*) }
    .sum
end
...
solveStep1(TEST_INPUT)
solveStep1(INPUT)

solveStep2(TEST_INPUT)
solveStep2(INPUT)
