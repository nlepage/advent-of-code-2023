...
def solve(input)
  ...
  h,w=lines.length,lines[0].length

  y = lines.index { |line| line.include? 'S' }
  x = lines[y].index('S')

  d = case
  when y>0 && 'F7|'.include?(lines[y-1][x])
    'N'
  when y<h-1 && 'LJ|'.include?(lines[y+1][x])
    'S'
  when x>0 && 'FL-'.include?(lines[y][x-1])
    'W'
  else
    'E'
  end
  ...
  while !q.empty?
    ...
    if y>-1 && !v.include?([y-1,x]) && !l.include?([y-1,x])
      q << [y-1,x]
      v << [y-1,x]
    end
    if y<h*2-1 && !v.include?([y+1,x]) && !l.include?([y+1,x])
      q << [y+1,x]
      v << [y+1,x]
    end
    if x>-1 && !v.include?([y,x-1]) && !l.include?([y,x-1])
      q << [y,x-1]
      v << [y,x-1]
    end
    if x<w*2-1 && !v.include?([y,x+1]) && !l.include?([y,x+1])
      q << [y,x+1]
      v << [y,x+1]
    end
  end

  p w*h - l.count { |y,x| y.even? && x.even? } - v.count { |y,x| y.even? && x.even? }
end
